package pine.toast.library

import org.bukkit.NamespacedKey

object WonderlandKeys {

    val ITEM_HANDLER = NamespacedKey("wonderland", "item_handler")
    val ENTITY_HANDLER = NamespacedKey("wonderland", "entity_handler")

    val COOLDOWN = NamespacedKey("wonderland", "cooldown_end_time")

    val ENCHANT_LEVEL = NamespacedKey("wonderland", "enchant_level")
    val ENCHANTMENT_STORAGE = NamespacedKey("wonderland", "enchantment_storage")
    val ENCHANTMENT_BOOK = NamespacedKey("wonderland", "enchantment_book")
}